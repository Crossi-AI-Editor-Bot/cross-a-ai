...
import { isOpenRouterModel, isMagicHourModel, isMagicHourImage, isMagicHourVideo, isMagicHourAudio } from "@/lib/externalModels";
    ...
    <>
      ...
      <div className="flex-1 overflow-auto p-4 font-mono text-sm">
        <div className="space-y-4 bg-muted/30 rounded-lg p-4 border border-border">
          ...
          {/* Max Tool Calls - applies to every model */}
          {onUpdateMaxToolCalls && (
            <div className="flex items-center gap-2">
              <span className="text-purple-400">max_tool_calls</span>
              <span className="text-muted-foreground">=</span>
              <Input
                type="number"
                step="1"
                min="0"
                max="20"
                value={model.max_tool_calls ?? 3}
                onChange={(e) => onUpdateMaxToolCalls(Math.max(0, parseInt(e.target.value, 10) || 0))}
                className="h-7 w-24 text-sm font-mono bg-background text-orange-400"
              />

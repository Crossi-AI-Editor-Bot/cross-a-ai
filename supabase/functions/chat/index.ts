...
import { loadUserConnector, buildConnectorToolLines, buildConnectorRegex, runConnectorTool, type ConnectorRow } from '../_shared/connectorTools.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const toolFlags = {
      ...
      connectors: (modelCostData as any).tool_connectors !== false,
    };
    ...
    // Google connector (Gmail/Drive) this user has linked, if any.
...
- /!present_fs_file <path>                 — hand a file from the sandbox terminal to the user as a downloadable card (works for zips and other binaries). Example: /!present_fs_file bundle.zip`);
    ...
    const connectorToolLines = toolFlags.connectors ? buildConnectorToolLines(userConnector) : [];
    if (connectorToolLines.length) optionalToolLines.push(...connectorToolLines);
...
You may call multiple tools in one turn (one per line). Do NOT explain that you are calling a tool — just emit the command.`;
    ...
    const CONNECTOR_TOOL_RE = toolFlags.connectors ? buildConnectorRegex(userConnector) : null;
    // Matches every recognized tool invocation line (built-in, addon, or connector), in the order they appear.
    const matchAllTools = (content: string): string[] => {
      ...
      if (CONNECTOR_TOOL_RE) for (const m of content.matchAll(CONNECTOR_TOOL_RE)) spans.push({ index: m.index ?? 0, text: m[0] });
    ...
    const stripAllToolLines = (content: string): string => {
      ...
      if (CONNECTOR_TOOL_RE) out = out.replace(CONNECTOR_TOOL_RE, "");
      return out;
    };
    ...
    const runTool = async (raw: string): Promise<ToolResult> => {
      ...
      // ---- Connector tools: /!gmail:read, /!gmail:write, /!drive:read -------
      const connectorMatch = line.match(/^\/!(gmail:read|gmail:write|drive:read)\s*(.*)$/i);
      if (connectorMatch) {
        ...
        return await runConnectorTool(serviceClient, { toolName: toolName.toLowerCase() as "gmail:read" | "gmail:write" | "drive:read", argsStr, connector: userConnector });
      }

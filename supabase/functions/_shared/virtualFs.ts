...
import { runPythonSandbox, pipInstall, type PyFile } from './pythonRuntime.ts';
...
// Apply a Pyodide sandbox filesystem snapshot back onto the virtual filesystem.
function applySnapshot(fs: Map<string, FsNode>, files: PyFile[]): string[] {
  const msgs: string[] = [];
  ...
  for (const f of files) {
    ...
    if (!f.isDir && f.bytes.length > TERMINAL_MAX_FILE_BYTES) {
      msgs.push(`'${f.path}': file too large to save (max ${Math.round(TERMINAL_MAX_FILE_BYTES / 1024)} KB)`);
      continue;
    }
    if (++entries > TERMINAL_MAX_ENTRIES) {
      msgs.push(`filesystem entry limit reached (${TERMINAL_MAX_ENTRIES}); some files were not saved`);
      break;
    }
    kept.add(f.path);
    if (f.isDir) {
      fs.set(f.path, { path: f.path, contentBase64: '', isBinary: false, isDir: true });
      continue;
    }
    let isBinary = true;
    try { strict.decode(f.bytes); isBinary = false; } catch { isBinary = true; }
    fs.set(f.path, { path: f.path, contentBase64: bytesToB64(f.bytes), isBinary, isDir: false });
  }
  for (const key of [...fs.keys()]) if (!kept.has(key)) fs.delete(key);
  return msgs;
}
